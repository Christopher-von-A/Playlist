...
Node **load_file(const char *filename, Node **list) {
  ...
  while (
    ...
    remove_newline_if_exists(line); // Remove newline from the line
    ...
    if (*list == NULL) {
      *list =
          new_node; // If the list is empty, set new node as the first element
    } else {
      tail(*list)->next = new_node; // The list has elements, append at the end
    }
  }
  fclose(f);
  return list;
...
void save_file(const char *filename, Node *list) {
  ...
  // YOUR CODE HERE
  while (current != NULL) {
    // Write the track to the file with a newline
    fprintf(f, "%s\n", *(Data *)current->data);
